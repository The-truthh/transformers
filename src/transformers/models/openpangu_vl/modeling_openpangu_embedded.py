...
class PanguEmbeddedModel(PanguEmbeddedPreTrainedModel):
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        # =================================================================
        # 【NEW LOGIC: Create 2D text position IDs for Masking】
        # The 3D position_ids are required for mROPE (spatial/temporal encoding).
        # However, create_causal_mask in the transformers library only expects 2D IDs [B, S]
        # or None for standard causal masking.
        # =================================================================
        
        # the hard coded `3` is for temporal, height and width. (T, H, W for multi-resolution RoPE)
        if position_ids is None:
            # If position IDs are not provided, generate the required 3D tensor for RoPE.
            position_ids = cache_position.unsqueeze(0)
        elif position_ids.ndim == 2:
            # If only a standard 2D tensor is provided, expand it to the required 3D tensor for RoPE.
            position_ids = position_ids[None, ...].expand(3, position_ids.shape[0], -1)
        
        if position_ids.ndim == 3 and position_ids.shape[0] == 4:
            # Scenario 1: Packed sequence data (D=4). The first dimension (index 0) is reserved for the 2D text ID.
            text_position_ids = position_ids[0]
            
            # The remaining 3 dimensions (T, H, W) are kept for RoPE calculation in the attention layer.
            position_ids = position_ids[1:]
            
        elif position_ids.ndim == 3 and position_ids.shape[0] == 3:
            # Scenario 2: Standard 3D mROPE data (OpenPangu-VL default). 
            # This tensor is only for spatial encoding and lacks a dedicated 2D text ID dimension.
            # We set it to None, which tells create_causal_mask to use a default incremental sequence, 
            # correctly derived from attention_mask and sequence length.
