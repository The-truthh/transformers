...
try:
    ...
    NPU_ATTN_INFR = hasattr(torch, "npu") and "910" in torch.npu.get_device_name()
...
def rescale_and_normalize(
    ...
    do_normalize: bool,
    image_mean: Union[float, list[float], tuple[float, ...]],
    image_std: Union[float, list[float], tuple[float, ...]],
) -> torch.Tensor:
    if do_rescale:
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not self.training and NPU_ATTN_INFR:
            ...
            q, k, v = [
                x.permute(0, 2, 1, 3).reshape(-1, x.shape[1], x.shape[3])
...
class OpenPanguPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["PanguEmbeddedDecoderLayer", "OpenPanguVLVisionBlock"]
    ...
    def forward(self, x, position_ids):
        ...
        with torch.autocast(device_type=device_type, enabled=False):  # Force float32
            ...
            if self.mrope_interleaved:
                ...
                # [NPU Fix] Workaround for Ascend StridedSlice alignment bug (EZ9999/507035).
                # Offload split/slice operations to CPU to avoid memory alignment issues.
                target_device, target_dtype = emb.device, emb.dtype

                # 1. Move to CPU (using float32 for maximum compatibility)
                emb_cpu = emb.to("cpu").to(torch.float32)

                # 2. Perform split and slice operations on CPU
                parts = []
                for i, m in enumerate(emb_cpu.split(mrope_section_3d, dim=-1)):
                    idx = mrope_dim[i]
                    if isinstance(idx, torch.Tensor):
                        idx = idx.to("cpu")
                    parts.append(m[idx])

                # 3. Concatenate and move back to the original device
                emb = torch.cat(parts, dim=-1).to(target_device).to(target_dtype)
...
class OpenPanguVLModel(OpenPanguPreTrainedModel):
    ...
    def _parse_preprocess_params(self, vision_config):
        ...
        self.do_rescale = vision_config.do_rescale
        self.rescale_factor = vision_config.rescale_factor
        self.do_normalize = vision_config.do_normalize
        self.image_mean = tuple(vision_config.image_mean)
        self.image_std = tuple(vision_config.image_std)
    ...
    def get_rope_index(
        ...
        Global Grid Indexing: Fixes bug where indices reset for each sample.
        """
        ...
        if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
            ...
            # =================================================================
            # [FIX] Global pointer initialization (moved outside the loop)
            # =================================================================
            global_video_idx = 0
            global_image_idx = 0
            # =================================================================
            ...
            for i, input_ids in enumerate(total_input_ids):
                # Note: input_ids here shadows the argument, represents the current row
                input_ids = input_ids[attention_mask[i] == 1]
                input_tokens = input_ids.tolist()
                src_item = input_tokens
                
                # [FIX] Local video_idx and image_idx reset removed      
                ...
                while idx < len(src_item):
                    ...
                    if src_item[idx] not in [video_token_id, image_token_id]:
                        ...
                        # [FIX] Use global_image_idx
                        grid_t = image_grid_thw[global_image_idx][0]
                        grid_hs = image_grid_thw[:, 1]
                        grid_ws = image_grid_thw[:, 2]
                        t_index = (torch.arange(grid_t) * 1 * tokens_per_second).long()
                        
                        llm_pos_ids = self._get_llm_pos_ids_for_vision(
                            start_idx, global_image_idx, spatial_merge_size, t_index, grid_hs, grid_ws
                        )
                        llm_pos_ids_list.append(llm_pos_ids.to(position_ids.device))
                        vision_seqlen = image_grid_thw[global_image_idx].prod() // (spatial_merge_size**2)
                        new_src_item.extend([image_token_id] * vision_seqlen)
                        
                        # [FIX] Increment global pointer
                        global_image_idx += 1
                    
                    else:
                        # src_item[idx] == video_token_id
                        # [FIX] Use global_video_idx
                        T = video_grid_thw[global_video_idx][0].item()
                        H = video_grid_thw[global_video_idx][1].item()
                        W = video_grid_thw[global_video_idx][2].item()
    ...
    def _get_llm_pos_ids_for_vision(
        ...
        """
        [DEBUG MODIFIED] Vision Position ID generation function with detailed printing
        """
        import os
        is_rank0 = int(os.environ.get("RANK", "0")) == 0
        
        # 1. Get original grid height and width
        raw_h = grid_hs[vision_idx].item()
        raw_w = grid_ws[vision_idx].item()
        
        # 2. Calculate height and width after downsampling
        llm_grid_h = grid_hs[vision_idx] // spatial_merge_size
        llm_grid_w = grid_ws[vision_idx] // spatial_merge_size
        
        llm_pos_ids_list = []
        
        # Generate indices for H dimension
        h_index = (
        ...
        # Generate indices for W dimension
        w_index = (
        ...
        # Generate indices for T dimension
        t_index_tensor = (
...
class OpenPanguVLForConditionalGeneration(OpenPanguPreTrainedModel, GenerationMixin):
...
OpenPanguVL = OpenPanguVLForConditionalGeneration
